...
export function StakeConfirmDialog({
  ...
  async function handleConfirm() {
    ...
    // Best-effort local fetch: same provider-stub that signed the tx
    // has it immediately, no propagation latency. Swallow any error so
    // the server slow-path can take over without the UI crashing here.
    ...
    const body: { txHash: `0x${string}`; receipt?: Record<string, unknown> } = receipt
      ? { txHash, receipt }
      : { txHash };
    try {
