...
**Implementado, no validado en device físico (2026-07-02)**: el flow Arena end-to-end (deposit + match + settle). El usuario confirmó que el Load Test Page de MiniPay sí le permite firmar transacciones en otros juegos, pero 6 shapes distintos de `eth_sendTransaction` con `feeCurrency` USDT revierten con `eth_estimateGas: "execution reverted"` (data `0x`) en este device. La causa raíz es probablemente el provider-stub del dev mode de MiniPay, no el código del flow (que cumple exactamente con docs.minipay.xyz/technical-references/send-transaction). El código está commiteado (commits `c1495c5`/`6f9ebb9`/`9275b5f`/`da67002`/`2710924`/`3e4d5ad`/`563ca3f`/`df3e5c0`/`db281d0`), tests pasan, server `/api/deposit` está vivo, treasury address configurada. El flow se validará end-to-end cuando la Mini App se someta al catálogo real de MiniPay (Stage 2) y se pruebe en producción, no en el Load Test Page.

**Único pendiente real**: registro en talent.app (task 5.6) — acción manual del usuario. Submission a MiniPay catalogue (Stage 2: UI screenshot 360×640, PageSpeed ≥90, ToS/Privacy, 24h SLA) sigue fuera del scope del MVP pero es ahora el siguiente paso natural.
