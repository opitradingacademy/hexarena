...
Deposit (Approach B — chosen for MVP, validated 2026-07-03):
User → eth_sendTransaction via MiniPay provider-stub (CIP-64 type 'cip64', feeCurrency: USDT adapter)
→ viem waits for tx to be signed and broadcast via the provider-stub
→ client fetches the receipt via eth_getTransactionReceipt (provider-stub has it instantly)
→ POST /api/deposit { txHash, receipt } → server validates structurally
(status='success', to=treasury, Transfer event matching) → creditDeposit(store, userId, txHash, amount)
→ ledger.credit(userId, amount) → internal balance

The "deposit + client-fetched receipt" pattern is critical because:

- Public Celo RPCs (forno.celo.org, celo-rpc.publicnode.com) have 2-30s propagation latency for new tx hashes.
- The MiniPay provider-stub that signed the tx sees the receipt in milliseconds.
- Server never polls public RPCs in the happy path; polling remains a fallback for clients that can't provide the receipt themselves.
...
function applyMove(
  ...
  p: PlayerId,
  at: Axial,
): { state: GameState; captures: Axial[] } | { error };
