...
- [x] 5.5.4 `apps/web/lib/balance.ts` + `app/page.tsx` — Dashboard reads real USDT balance via viem `publicClient.readContract` (Celo Mainnet, `SETTLEMENT_TOKEN_ADDRESS[42220]`, 6 decimals), loading state, $0.00 fallback when no wallet.
- [ ] 5.5.5 **OPEN BUG, unresolved**: Dashboard balance shows $0.00 on the real physical MiniPay device tested (via MiniPay's Developer Mode "Load Test Page" / "Mini App Test" tool) despite `getWalletAddress()` matching a reference Mini App confirmed working on the SAME device with the SAME wallet (verified showing the real $3.91 USDT balance side by side, screenshots on file). See "Known Issues" note below — needs either physical-device remote debugging (chrome://inspect via USB, blocked so far by a pairing-approval popup that never appeared) or further comparison against the working reference app's full script-load/execution order, not just its wallet-read function in isolation.
...
- **MiniPay Dashboard balance shows $0.00 on physical device** despite the wallet-read logic matching a proven-working reference app tested on the exact same device/wallet. Multiple root causes were found and fixed along the way (all confirmed real, all still valid fixes, kept in place): (1) MiniPay's `window.ethereum` injects asynchronously — added `apps/web/lib/waitForEthereum.ts` to wait for the `ethereum#initialized` event; (2) the wallet address read now matches the reference app's exact `eth_requestAccounts` call (`apps/web/lib/wallet.ts`, simplified from an earlier viem-`getAddresses()` + legacy-`enable()` fallback chain that was also intermittent). Despite all of this, the bug persists as of the last test. Next debugging step needs either successful physical-device remote debugging (USB pairing popup did not appear when last attempted) or a byte-for-byte comparison of the reference app's full page load/script execution order, since its wallet connect is a plain top-level `<script>` tag (not a Next.js client component behind React hydration) — that structural difference in load timing has not been ruled out yet.
