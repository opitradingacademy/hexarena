...
- [x] 5.5.6 Arena end-to-end deposit flow — `apps/server/depositEndpoint.ts` + `apps/server/chain/verifyDeposit.ts` (server-side POST /api/deposit with on-chain receipt verification, idempotent by tx_hash) + `apps/web/lib/transferUsdt.ts` (raw eth_sendTransaction with USDT adapter feeCurrency, MiniPay-compliant shape per docs.minipay.xyz) + `apps/web/components/StakeConfirmDialog.tsx` + MatchmakingScreen wiring. **Status: ✅ VERIFIED on physical MiniPay device 2026-07-03**. User successfully signed a tx in the MiniPay WebView, transaction was confirmed on Celo Mainnet (CIP-64 type 0x7b, fee paid in USDT 0.001383), and the modal closed correctly after the server validated the receipt. **Final fix stack** (8 deploys in chronological order): (1) refactored to viem `createWalletClient` + `custom(window.ethereum)` transport with `type: 'cip64'` — the canonical shape per docs.celo.org. (2) Validated treasury address at boot (`apps/server/indexEnv.ts` + `apps/web/lib/serverUrl.ts`) — found that the user's NEXT_PUBLIC_ARENA_TREASURY_ADDRESS was a 32-byte (64-hex) corrupted value, NOT a valid 20-byte Ethereum address. (3) Added CORS preflight + Access-Control-Allow-Origin on every response (`apps/server/cors.ts`) — the cross-origin POST from Vercel to Railway was being silently blocked. (4) Switched primary RPC from forno.celo.org to celo-rpc.publicnode.com (fallback chain). (5) **Final structural fix**: client now fetches the receipt via MiniPay provider-stub (`eth_getTransactionReceipt`) and POSTs the full receipt to the server. Server validates structurally without RPC polling — public RPCs have 2-30s propagation delay, the provider-stub that signed sees the receipt in milliseconds. Polling kept as a fallback for clients that can't provide the receipt themselves. 159/159 tests pass.
...
- (resolved 2026-07-03) **MiniPay Arena deposit flow was unable to confirm receipts on the server** — root cause: public Celo RPCs (forno.celo.org, celo-rpc.publicnode.com) have 2-30s propagation latency for newly broadcast tx hashes, while the user's MiniPay provider-stub sees the receipt in milliseconds (it was the nodo that processed the broadcast). Fix: client now fetches the receipt locally and POSTs the full receipt to /api/deposit; server validates structurally. Polling kept as a fallback only. Commits `194d759`, `871610f`, `6502182`.
- (resolved 2026-07-03) **Treasury address corruption** — `NEXT_PUBLIC_ARENA_TREASURY_ADDRESS` was a 32-byte (64-hex-char) value, NOT a valid 20-byte (40-hex) Ethereum address. The recipient encoded in the tx's calldata was therefore an invalid address and the on-chain `eth_estimateGas` simulation reverted with bare 'execution reverted, data: 0x'. Fix: validate treasury at boot in `apps/server/indexEnv.ts` (server) and `apps/web/lib/serverUrl.ts` (client) with explicit error messages. Commit `2dccefd`.
- (resolved 2026-07-03) **CORS preflight blocking cross-origin POST** — Socket.IO's CORS doesn't extend to plain HTTP handlers. The WebView's browser was silently blocking the cross-origin POST /api/deposit from Vercel to Railway. Fix: `apps/server/cors.ts` + OPTIONS preflight + Access-Control-Allow-Origin on every response. Commit `ef3e9a2`.
